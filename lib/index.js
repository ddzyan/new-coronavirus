  ...
  static async saveNewCoronavirusDate() {
    ...
    try {
      ...
      const saveCountryResult = await daos.CountryDateDao.findOrCreateCountryDate({
        createdAt, confirmedCount: diagnosed, suspectedCount: suspect, deadCount: death, curedCount: cured,
      });
      ...
      if (saveCountryResult) {
        ...
        for (const areaItem of area) {
          ...
          // 保存省份病毒信息
          await daos.AreaDateDao.findOrCreateAreaDate({ createdAt, areaId, ...areaItem });
          ...
          logger.debug(`${provinceName}省份昨日病毒增长信息成功`);
        }
      }
    } catch (error) {
